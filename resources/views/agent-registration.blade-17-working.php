...
document.getElementById('agentForm1').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showAlert(data.message || 'Step 1 failed', 'error');
        }
    } catch (error) {
...
document.getElementById('agentForm2').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showAlert(data.message || 'Step 2 failed', 'error');
        }
    } catch (error) {
        showAlert('An error occurred. Please try again.', 'error');
    } finally {
...
async function handlePayment() {
    ...
    try {
        ...
        if (data.success) {
            if (data.test_payment) {
                ...
                const options = {
                    ...
                    "description": "Early Bird Registration - ₹999",
                    "image": "/favicon.ico",
        ...
        paymentBtn.textContent = 'Pay ₹999 & Register';
    }
}
...
async function handlePaymentSuccess(paymentResponse, agentId) {
    try {
        ...
        if (data.success) {
            showAlert('🎉 Registration Complete! Welcome to Padosi!', 'success');
            setTimeout(() => {
                window.location.reload();
            }, 3000);
        } else {
